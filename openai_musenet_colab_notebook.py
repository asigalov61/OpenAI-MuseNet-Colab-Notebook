...
Original file is located at
    https://colab.research.google.com/drive/1BrxYyzrvqLNXxrC1b-jopWtUBg7VHyZ7

# MuseNet Companion (ver 0.8)
...
itrack = 1 
while itrack < len(data1):
    for event in data1[itrack]:
        if event[0] == 'note':
...
for event in events_matrix2:
   time = event[1]
   encoded.append(min(127, int(time / 10)) + (128 * 31))
...
number_of_ticks_per_quarter = data[0]
...
for token in encoding:
    if 0 <= int(token) < 3840:
        ...
        channel = TRACKS_OUT_INDEX[INSTRUMENTS[inst_vol_index]]
        delay = delta_times
        if velocity > 0:
          song.append(['note_on', delay * DELAY_MULTIPLIER, channel, note, velocity])
